# USAGE
# python PGD_basic.py --input pig.jpg --output adversarial.png --delta delta.png --class-idx 341
...
def generate_adversaries(model, baseImage, delta, classIdx, steps=50):
	...
	return delta
def generate_adversaries2(model, baseImage, delta, classIdx, steps=50):
	# iterate over the number of steps
	for step in range(0, steps):
		# record our gradients
		with tf.GradientTape() as tape:
...
deltaUpdated = generate_adversaries2(model, baseImage, delta,
	args["class_idx"])
