# USAGE
# python PGD_targeted.py --input pig.jpg --output adversarial.png --delta delta.png --class-idx 341 --target-idx 342
...
def generate_adversaries(model, baseImage, delta, classIdx, targetIdx, steps=50):
	...
	for step in range(0, steps):
		...
		with tf.GradientTape() as tape:
			...
			loss = -1/2 * sccLoss(tf.convert_to_tensor([classIdx]),predictions) + 1/2 * sccLoss(tf.convert_to_tensor([targetIdx]),
				predictions)
#
...
for j in range(1):
	...
	deltaImage = (deltaUpdated).numpy().squeeze()
	deltaImage = np.clip(deltaImage*100, 0, 255).astype("uint8")
